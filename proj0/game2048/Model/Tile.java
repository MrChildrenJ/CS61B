...
public class Tile {
    ...
    /** Return the result of moving me to (COL, ROW). Just moving, nothing with merge. */
    ...
    public Tile merge(int col, int row, Tile otherTile) {
        assert this.value == otherTile.value();      // values of two tiles need to be equal
        this.next = otherTile.next = new Tile(2 * value, col, row);
        return next;
    }
